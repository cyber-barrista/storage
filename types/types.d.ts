...
export declare type BucketConfig<T> = {
    bucketName: string;
    areaName: AreaName;
    defaultValue: T;
};
export declare type CoreGetter<T> = Extract<keyof T, string> | Extract<keyof T, string>[] | Partial<T>;
export declare type Getter<T> = Extract<keyof T, string> | Extract<keyof T, string>[] | ((values: T) => any) | Partial<T>;
...
export interface Bucket<T extends object> {
    ...
    get<P extends Extract<keyof T, string>>(getter: P): Promise<{
        [key in P]: T[P];
    }>;
    get<P extends Extract<keyof T, string>[]>(getter: P): Promise<Partial<T>>;
