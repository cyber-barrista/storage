...
function getBucket(bucketName, areaName) {
    ...
    const nativeChange$ = rxjs.fromEventPattern((handler) => {
        chrome.storage.onChanged.addListener(handler);
    }, (handler) => {
        chrome.storage.onChanged.removeListener(handler);
    });
    const changeStream = nativeChange$.pipe(operators.filter(([changes, area]) => {
    ...
    return {
        ...
        get valueStream() {
            return rxjs.concat(rxjs.from(get()), changeStream.pipe(operators.mergeMap(() => get())));
        },
    };
}
